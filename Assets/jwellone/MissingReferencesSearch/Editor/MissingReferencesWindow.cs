...
namespace jwelloneEditor
{
	public class MissingReferencesWindow : EditorWindow
	{
		...
		[MenuItem("Tools/jwellone/window/MissingReferencesWindow")]
		static void Open()
		{
	...
	void OnFindProject()
	{
		...
		{
			var path = AssetDatabase.GUIDToAssetPath(guids[i]);
			if (EditorUtility.DisplayCancelableProgressBar("Searching Prefabs", path, (i + 1) / (float)guids.Length))
			{
				cancelled = true;
				break;
			}
			var data = new MissingReferences(guids[i]);
			if (data.isMissing)
			{
		...
		if (!cancelled)
		{
			...
			EditorUtility.DisplayDialog("Missing Search", $"There are {count} missing part.", "close");
		}
		else
		{
			EditorUtility.DisplayDialog("Missing Search", "Search cancelled.", "close");
		}
	}
	...
	void OnFindHierarchy()
	{
		...
		int total = allGameObjects.Length;
		bool cancelled = false;
		for (int i = 0; i < total; i++)
		{
			var go = allGameObjects[i];
			string info = $"Checking: {go.name} ({i + 1}/{total})";
			if (EditorUtility.DisplayCancelableProgressBar("Searching Hierarchy", info, (float)i / total))
			{
				cancelled = true;
				break;
			}
			var data = new MissingReferences(go);
			if (data.isMissing)
			{
				_hierarchyReferences.Add(data);
			}
		}
		EditorUtility.ClearProgressBar();
