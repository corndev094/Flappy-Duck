namespace Multiplayer
{
    ...
    public class MatchmakingManager : NetworkBehaviour
    {
        ...
        [SerializeField] private float gracePeriod = 10f; // Wait time after min players for more to join
        ...
        private const string KEY_MIN_LEVEL = "N1";
        private const string KEY_MAX_LEVEL = "N2";
        private const string KEY_REGION = "S2";
        ...
        public async UniTask<MatchingResult> StartQuickMatchmaking()
        {
            ...
            catch (Exception e)
            {
                ...
                try
                {
                    ...
                    await CreateLobbyAndHost(MatchmakingFilter.Default);
        ...
        #region Matchmaking with Filters

        /// <summary>
        /// Start matchmaking with specific conditions
        /// </summary>
        public async UniTask<MatchingResult> StartMatchmaking(MatchmakingFilter filter)
        {
            if (IsMatchingInProgress)
            {
                Debug.LogWarning("[Matchmaking] Already in progress");
            ...
            try
            {
                ...
                var lobbies = await FindMatchingLobbies(filter);
                ...
                await CreateLobbyAndHost(filter);
        ...
        /// </summary>
        private async UniTask<List<Lobby>> FindMatchingLobbies(MatchmakingFilter filter)
        {
            var queryOptions = new QueryLobbiesOptions
            {
                ...
                Filters = new List<QueryFilter>
                {
                    ...
                    new(QueryFilter.FieldOptions.S1, ((int)filter.GameMode).ToString(), QueryFilter.OpOptions.EQ),
                    new(QueryFilter.FieldOptions.N1, filter.PlayerLevel.ToString(), QueryFilter.OpOptions.LE),
                    new(QueryFilter.FieldOptions.N2, filter.PlayerLevel.ToString(), QueryFilter.OpOptions.GE)
                },
            ...
            {
                queryOptions.Filters.Add(
                    new QueryFilter(QueryFilter.FieldOptions.S2, filter.Region, QueryFilter.OpOptions.EQ)
                );
            }
        ...
        private async UniTask CreateLobbyAndHost(MatchmakingFilter filter)
        {
            ...
            int minLevel = filter.PlayerLevel - filter.LevelRange;
            int maxLevel = filter.PlayerLevel + filter.LevelRange;

            // Create lobby data
            var lobbyData = new Dictionary<string, DataObject>
            {
                { KEY_JOIN_CODE, new DataObject(DataObject.VisibilityOptions.Member, joinCode) },
                { KEY_GAME_MODE, new DataObject(DataObject.VisibilityOptions.Public, ((int)filter.GameMode).ToString(), DataObject.IndexOptions.S1) },
                { KEY_MIN_LEVEL, new DataObject(DataObject.VisibilityOptions.Public, minLevel.ToString(), DataObject.IndexOptions.N1) },
                { KEY_MAX_LEVEL, new DataObject(DataObject.VisibilityOptions.Public, maxLevel.ToString(), DataObject.IndexOptions.N2) },
            };

            if (!string.IsNullOrEmpty(filter.Region))
            {
                lobbyData[KEY_REGION] = new DataObject(
            ...
            var createOptions = new CreateLobbyOptions
            {
                IsPrivate = false,
                Data = lobbyData,
                Player = CreatePlayerData(filter.PlayerLevel)
            };

            CurrentLobby = await LobbyService.Instance.CreateLobbyAsync(
                $"Match_{filter.GameMode}_{filter.PlayerLevel}",
                maxPlayer,
                createOptions
            );
        ...
        private Player CreatePlayerData(int level)
        {
            return new Player
            {
    ...
    [Serializable]
    public struct MatchmakingFilter
    {
        ...
        public static MatchmakingFilter Default => new()
        {
